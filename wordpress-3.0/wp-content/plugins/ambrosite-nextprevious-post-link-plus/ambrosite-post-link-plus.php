...
//		If there is no next/previous post, loop back around to the first/last post.		
		...
		$where = apply_filters( "get_{$adjacent}_post_where", $wpdb->prepare("WHERE {$order_by} $op {$order_format} AND p.post_type = %s AND (p.post_status = 'private' OR p.post_status = 'publish') $posts_in_ex_cats_sql", $current_post, $post->post_type), $in_same_cat, $excluded_categories );
